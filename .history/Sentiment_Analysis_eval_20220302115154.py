...
def val_accuary(model, val_dataloader, device, criterion):
    ...
    with torch.no_grad():
        ...
        for j, val_data in enumerate(val_dataloader, 0):
            ...
            output1 = model(input_val)
            loss1 = criterion(output1, target_val)
            val_loss += loss1.item()
            _, predicted1 = torch.max(output1, 1)
            total += target_val.size(0)
            correct += (predicted1 == target_val).sum().item()
            F1 = f1_score(target_val.cpu(), predicted1.cpu(), average="weighted")
            Recall = recall_score(target_val.cpu(), predicted1.cpu(), average="micro")
            CM = confusion_matrix(target_val.cpu(), predicted1.cpu())
        print(
