...
def test_accuary(model, test_dataloader, device):
    ...
    with torch.no_grad():
        ...
        for k, data_test in enumerate(test_dataloader, 0):
            ...
            F1 = f1_score(target_.cpu(), pred_test.cpu(), average="weighted")
            Recall = recall_score(target_.cpu(), pred_test.cpu(), average="micro")
            CM = confusion_matrix(target_.cpu(), pred_test.cpu())
        print(
