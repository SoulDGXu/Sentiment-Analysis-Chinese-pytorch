...
def val_accuary(model, val_dataloader, device, criterion):
    ...
    with torch.no_grad():
        ...
        for j, val_data in enumerate(val_dataloader, 0):
            input_, target_ = val_data[0], val_data[1]
            ...
            correct += (predicted1 == target_).sum().item()
            F1 = f1_score(target_.cpu(), predicted1.cpu(), average="weighted")
            Recall = recall_score(target_.cpu(), predicted1.cpu(), average="micro")
            CM = confusion_matrix(target_.cpu(), predicted1.cpu())
        print(
