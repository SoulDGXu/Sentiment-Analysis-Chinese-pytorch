...
class LSTMModel(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # states: (seq_len, batch, D*hidden_size), D=2 if bidirectional = True else 1
        # hidden: (h_n, c_n) => h_n / c_n shape:(D∗num_layers, batch, hidden_size)
        encoding = torch.cat(
            [states[0], states[-1]], dim=1
        )  # (batch, 2*D*hidden_size)[32,512]
