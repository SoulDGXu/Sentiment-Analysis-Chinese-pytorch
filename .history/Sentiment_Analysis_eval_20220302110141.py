...
def val_accuary(model, val_dataloader, device, criterion):
    ...
    with torch.no_grad():
        correct1 = 0  # 预测的和实际的label相同的样本个数
        total1 = 0  # 累计validation样本个数
        val_loss = 0.0
        ...
        print(
            "\nVal accuracy : {:.3f}%,val_loss:{:.3f}, F1_score：{:.3f}%, Recall：{:.3f}%".format(
                100 * correct1 / total1, val_loss, 100 * F1, 100 * Recall
            )
        )
...
def test_accuary(model, test_dataloader, device):
    ...
    - device: str, choose torch device {"cpu", "cuda"}
    @return {*}
    test_accuracy: the accuracy of total validation set
    """
    model = model.to(device)
    with torch.no_grad():
        correct = 0
...
if __name__ == "__main__":
    ...
    model = torch.load("./word2vec/sen_model_best.pkl")
